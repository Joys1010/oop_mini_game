...
private:
	char tempMap[19][56]; // 맵 임시저장할 배열  

	char map1[19][56] = { // 쉬운맵  
		// 가로 숫자는 위 배열크기에 맞추어 56개  
		{"1111111111111111111111111111111111111111111111111111111"},
		{"10l0000000000000111000nhhhnhhhnhhhnhhhn0088800000000001"},    
		{"10l0000000000000111000n2hhn3hhn4kkn5hhn0088800nnn00nnn1"}, // 1 : 장애물
		{"10000000000000001110000000000000000000000887002hh002hh1"}, // 6 : B603
		{"10l0000000000000111000nhhhnhhhnhhhnhhhn0088800hhh00hhh1"}, // 7 : 문 
		{"10l0000000000000111000n2hhn3hhn4hhn5hhn0000000nnn00nnn1"}, // l : 엘레베이터 (이자 열쇠획득 시 탈출구)  // 다음 stage로 가는 길!
		{"10000000000000000000000000000000000000000000003hh003hh1"}, // 2 : 1번 사물함 ,3-2번 사물함, 4-3번 사물함, 5-4번 사물함
		{"1111000000000000000000000000000000000000000000hhh00hhh1"}, // 8 : toilet
		{"1111000000000111111111000000000000000000000000nnn00nnn1"}, // b : 다시 돌아오는 지점1
		{"11110000000001111111110000000000066666666600004hh004hh1"}, // s : 플레이어
		{"1111000000000000000000000000000006666666660000hhh00hhh1"}, // 9 : 화장실 문
		{"1111000000n5hhn4hhn3hhn2hhn0000006666666660000nnn00nnn1"}, // h : 가로선
		{"1111000000nhhhnhhhnhkknhhhn0000006666666660000000000001"}, // n : 세로선
		{"1111000000000000000000000000000006666669990000000000111"}, // m : 사물함 문 여는 부분
		{"1111111100000000000000000000000006666666660000000000111"}, // k : 전공책1 (m이랑 구분되어야 함)
		{"1111111100000000000000000000000000000000000000000000111"}, // j : 전공책2
		{"1111111111111111100000011000000111111111111111111111111"}, // d : 전공책3
		{"00000000000000000x0000x00x0000x000000000000000000000000"}, //x : / 문
	...
	void drawMap(int* x, int* y) {
		...
		for (h = 0; h < 19; h++) { // 세로길이까지  
			for (w = 0; w < 56; w++) { // 가로길이까지  
				...
				else if (temp == 'x') { //사물함 번호 4
					setColor(white, black);
					cout << "|";
				}
				else if (temp == '6') { //사물함 번호 4
					setColor(yellow, yellow);
					cout << "|";
				}
				else if (temp == '7') { //사물함 번호 4
					setColor(brown, brown);
					cout << "|";
				}
				else if (temp == '8') { //사물함 번호 4
					setColor(lightred, lightred);
					cout << "|";
				}
				else if (temp == '9') { //사물함 번호 4
					setColor(red, red);
					cout << "|";
				}
			}
		...
		if (mapObject == '0') {
			...
			*key += 1; // 열쇠갯수 1증가 
			...
			if (*key == 1) {
				...
				gotoxy(37, 12); // 증감한 위치로 이동 후 플레이어 출력  
				cout << "◎";
				*x = 37; // 실제 좌표값으로 반영
				*y = 12;
				cout << " ";
				...
				gotoxy(3, 4); // 증감한 위치로 이동 후 플레이어 출력  
				cout << "◎";
				*x = 3; // 실제 좌표값으로 반영
				*y = 4;
				cout << " ";
